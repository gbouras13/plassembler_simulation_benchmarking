rule run_plassembler_1_thread_real_raven:
    input:
        l = get_long,
    ...
    params:
        ...
        db =PLASSEMBLER_DB,
        out_dir = os.path.join(PLASSEMBLER_OUTPUT_1_THREADS_RAVEN,"{sample}"),
        chrom = get_length
    benchmark:
    ...
    shell:
        ...
        -o {params.out_dir} -t {threads} -p {wildcards.sample} -c {params.chrom} -d {params.db} -f --use_raven
        '''

rule run_plassembler_8_threads_real_raven:
    input:
        l = get_long,
        short_one = get_short_one,
        short_two = get_short_two
    ...
    params:
        bindir = PLASSEMBLER_BIN,
        db =PLASSEMBLER_DB,
        out_dir = os.path.join(PLASSEMBLER_OUTPUT_8_THREADS_RAVEN,"{sample}"),
        chrom = get_length
    benchmark:
...
rule run_plassembler_16_threads_real_rvaen:
    input:
        l = get_long,
        short_one = get_short_one,
        short_two = get_short_two
    ...
    params:
        bindir = PLASSEMBLER_BIN,
        db =PLASSEMBLER_DB,
        out_dir = os.path.join(PLASSEMBLER_OUTPUT_16_THREADS_RAVEN,"{sample}"),
        chrom = get_length
    benchmark:
...
rule aggr_plassembler_real:
    """aggregate lr"""
    input:
        expand(os.path.join(PLASSEMBLER_OUTPUT_1_THREADS_RAVEN,"{sample}", "{sample}_plasmids.fasta"), sample = SAMPLES),
        expand(os.path.join(PLASSEMBLER_OUTPUT_8_THREADS_RAVEN,"{sample}", "{sample}_plasmids.fasta"), sample = SAMPLES),
        expand(os.path.join(PLASSEMBLER_OUTPUT_16_THREADS_RAVEN,"{sample}", "{sample}_plasmids.fasta"), sample = SAMPLES),
